...
public:
    ...
    //--- 메모리 ---
    // 순환 참조 후보를 먼저 검사·회수한 뒤, 완전히 빈 풀 페이지를 OS 로 돌려준다.
    // 반환값 = OS 로 돌려준 페이지 바이트(순환 객체 자체의 바이트는 포함하지 않는다).
    //
    // [호스트 의무 — 매 프레임 TrimMemory(false) 를 불러야 한다]
    // 이건 "여유가 있으면 부르는" 최적화가 아니라 **필수 호출**이다. 참조 카운팅은
    // 순환을 회수하지 못하므로, 순환에 걸린 컨테이너는 여기서만 회수된다. 그리고
    // 순환 후보 대기열을 비우는 곳도 여기뿐이다 — 부르지 않으면 후보가 계속 쌓여
    // 메모리가 단조 증가한다. 스크립트를 돌리는 루프라면 프레임마다 한 번 부를 것.
    //
    //  force=false : 매 프레임 부르는 증분 회수다. 보유 시간(기본 5초)이 지난 페이지를
    //                한 번에 SetTrimPagesPerCall() 장까지만 돌려주고 나머지는 다음
    //                호출로 넘긴다. 순환 후보도 max(16개, 후보 전체의 0.5%)만 검사한다.
    //                보유 시계는 페이지가 빈 순간이 아니라 내부 스윕이 그것을 처음 본
    //                순간부터 흐르므로, 실제 유지 시간은 보유 시간보다 조금 길 수 있다.
    //
    //                [비용] 페이지 회수 쪽은 호출당 상한이 있다(장수 예산). 순환 검사
    //                쪽은 **검사할 후보 개수에만** 상한이 있고, 후보 하나의 비용에는
    //                상한이 없다 — 후보에서 도달 가능한 컨테이너 그래프 전체를 훑기
    //                때문이다. 즉 후보가 큰 공유 상태(전역 테이블 등)에 닿으면 그 한
    //                번의 검사가 그래프 크기에 비례한다. 컨테이너를 아주 많이 들고
    //                도는 타이틀이라면 프레임 시간을 실제로 재보고, 필요하면 맵 전환
    //                같은 안전한 시점에 force=true 로 몰아서 처리하는 편이 낫다.
    //                후보 대기열이 비어 있고 빈 페이지도 없으면 즉시 반환한다.
    //
    //  force=true  : 순환 후보와 빈 페이지를 모두 예산·보유 시간 없이 전량 처리한다.
    //                맵 전환처럼 확실한 정리 시점용. 밀린 후보가 많을수록 오래 걸리므로
    //                프레임 중에 부르지 말 것.
